...
public class FaturamentoConsolidadoDetalhadoReportService {
	...
	public List<FaturamentoBrutoVO> filtrarFaturamentoDetalhado(PesquisaFaturamentoBrutoVO pesquisaFaturamentoBrutoVO) {
		...
		for (String historico : listaHistoricos) {
			...
			BigDecimal valorHistoricoContabil = getValorTotal(despesasContabeisHistorico, FaturamentoBrutoReportProjection::getValorBruto).negate();
			...
			if (ListUtil.isNullOrEmpty(pesquisaFaturamentoBrutoVO.getCodigoVeiculos())
				...
				Set<String> listaPlacas = despesasContabeisHistorico.stream().map(FaturamentoBrutoReportProjection::getVeiculo)
						.collect(Collectors.toSet());

				listaPlacas.addAll(despesasSociaisHistorico.stream().map(FaturamentoBrutoReportProjection::getVeiculo).collect(Collectors.toSet()));
				...
				for (String placa : listaPlacas) {
					...
					BigDecimal valorPlacaContabil = getValorTotal(despesasContabeisPlaca, FaturamentoBrutoReportProjection::getValorBruto).negate();
